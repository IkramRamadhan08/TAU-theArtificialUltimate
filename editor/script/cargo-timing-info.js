...
function getZedDataDir() {
  ...
  if (platform === "darwin") {
    // macOS: ~/Library/Application Support/Tau
    return path.join(os.homedir(), "Library", "Application Support", "Tau");
  } else if (platform === "linux" || platform === "freebsd") {
    // Linux/FreeBSD: $FLATPAK_XDG_DATA_HOME or XDG_DATA_HOME/tau
    if (process.env.FLATPAK_XDG_DATA_HOME) {
      return path.join(process.env.FLATPAK_XDG_DATA_HOME, "tau");
    }
    const xdgDataHome = process.env.XDG_DATA_HOME || path.join(os.homedir(), ".local", "share");
    return path.join(xdgDataHome, "tau");
  } else if (platform === "win32") {
    // Windows: LocalAppData/Tau
    const localAppData = process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local");
    return path.join(localAppData, "Tau");
  } else {
    // Fallback to XDG config dir
    const xdgConfigHome = process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config");
    return path.join(xdgConfigHome, "tau");
  }
}
