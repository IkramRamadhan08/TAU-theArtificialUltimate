...
use client::{TauLink, parse_tau_link};
...
pub enum OpenRequestKind {
    ...
    InstallSkill {
        /// Full `SKILL.md` contents embedded in a `tau://skill` share link.
        content: String,
    },
    DockMenuAction {
        index: usize,
...
impl OpenRequest {
    ...
    pub fn parse(request: RawOpenRequest, cx: &App) -> Result<Self> {
        ...
        for url in request.urls {
            if let Some(server_name) = url.strip_prefix("tau-cli://") {
                this.kind = Some(OpenRequestKind::CliConnection(connect_to_cli(server_name)?));
            } else if let Some(action_index) = url.strip_prefix("tau-dock-action://") {
                this.kind = Some(OpenRequestKind::DockMenuAction {
                    index: action_index.parse()?,
                });
            } else if let Some(file) = url.strip_prefix("file://") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("tau://file") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("tau://ssh") {
                let ssh_url = "ssh:/".to_string() + file;
                this.parse_ssh_file_path(&ssh_url, cx)?
            } else if let Some(extension_id) = url.strip_prefix("tau://extension/") {
                this.kind = Some(OpenRequestKind::Extension {
                    extension_id: extension_id.to_string(),
                });
            } else if let Some(session_id_str) = url.strip_prefix("tau://agent/shared/") {
            ...
            } else if let Some(agent_path) = url.strip_prefix("tau://agent") {
                this.parse_agent_url(agent_path)
            } else if url == "tau://" || url == "tau://open" || url == "tau://open/" {
                this.kind = Some(OpenRequestKind::FocusApp);
            } else if let Some(schema_path) = url.strip_prefix("tau://schemas/") {
                this.kind = Some(OpenRequestKind::BuiltinJsonSchema {
                    schema_path: schema_path.to_string(),
                });
            } else if url == "tau://settings" || url == "tau://settings/" {
                this.kind = Some(OpenRequestKind::Setting { setting_path: None });
            } else if let Some(setting_path) = url.strip_prefix("tau://settings/") {
                this.kind = Some(OpenRequestKind::Setting {
                    setting_path: Some(setting_path.to_string()),
                });
            } else if let Some(clone_path) = url.strip_prefix("tau://git/clone") {
                this.parse_git_clone_url(clone_path)?
            } else if let Some(commit_path) = url.strip_prefix("tau://git/commit/") {
                this.parse_git_commit_url(commit_path)?
            } else if url.starts_with("ssh://") {
                this.parse_ssh_file_path(&url, cx)?
            } else if let Some(zed_link) = parse_tau_link(&url, cx) {
                match zed_link {
                    TauLink::Channel { channel_id } => {
                        this.join_channel = Some(channel_id);
                    }
                    TauLink::ChannelNotes {
                        channel_id,
                        heading,
                    } => {
    ...
    fn parse_skill_install_url(&mut self, url: &str) -> Result<()> {
        // Format: tau://skill?data=<base64url of SKILL.md contents>
...
async fn open_local_workspace(
    ...
    // `std::env::current_dir()`, since the Tau app process's cwd is typically
...
mod tests {
    ...
    fn test_parse_malformed_skill_install_url_errors(cx: &mut TestAppContext) {
        ...
        let result = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["tau://skill?data=!!!notbase64".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn agent_url_with_prompt(prompt: &str) -> String {
        let mut serializer = url::form_urlencoded::Serializer::new("tau://agent?".to_string());
    ...
    fn test_parse_agent_url_with_trailing_slash(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["tau://agent/?prompt=hello".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn test_parse_focus_app_url(cx: &mut TestAppContext) {
        ...
        for url in ["tau://", "tau://open", "tau://open/"] {
            let request = cx.update(|cx| {
                OpenRequest::parse(
                    RawOpenRequest {
                        urls: vec![url.into()],
    ...
    fn test_parse_shared_agent_thread_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![format!("tau://agent/shared/{session_id}")],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn test_parse_shared_agent_thread_url_with_invalid_uuid(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["tau://agent/shared/not-a-uuid".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn test_parse_git_commit_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["tau://git/commit/abc123?repo=path/to/repo".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["tau://git/commit/def456?repo=path%20with%20spaces".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        cx.update(|cx| {
            assert!(
                OpenRequest::parse(
                    RawOpenRequest {
                        urls: vec!["tau://git/commit/abc123?repo=".into()],
                        ..Default::default()
                    },
                    cx,
                )
        ...
        let result = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["tau://git/commit/abc123?foo=bar".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    async fn test_open_workspace_with_nonexistent_files(cx: &mut TestAppContext) {
        ...
        // but tell Tau to add it to the current workspace
        open_workspace_file(
            path!("/root/file6.txt"),
            workspace::OpenOptions {
        ...
        // but tell Tau to NOT add it to the current workspace
        open_workspace_file(
            path!("/root/file7.txt"),
            workspace::OpenOptions {
                workspace_matching: workspace::WorkspaceMatching::None,
    ...
    fn test_parse_git_clone_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![
                        "tau://git/clone/?repo=https://github.com/tau-industries/tau.git".into(),
                    ],
                    ..Default::default()
                },
                cx,
        ...
        match request.kind {
            Some(OpenRequestKind::GitClone { repo_url }) => {
                assert_eq!(repo_url, "https://github.com/tau-industries/tau.git");
            }
    ...
    fn test_parse_git_clone_url_with_encoding(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![
                        "tau://git/clone/?repo=https%3A%2F%2Fgithub.com%2Fzed-industries%2Fzed.git"
                            .into(),
                    ],
                    ..Default::default()
                },
    ...
    async fn test_e2e_no_flags_no_windows_no_prompt(cx: &mut TestAppContext) {
        ...
        let (status, prompt_shown) = run_cli_with_tau_handler(
            cx,
            app_state,
