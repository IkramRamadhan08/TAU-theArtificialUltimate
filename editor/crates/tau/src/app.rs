...
use project::{DirectoryLister, DisableAiSettings};
...
pub fn init(cx: &mut App) {
    ...
    // When Tau logs to stdout rather than the log file, avoid registering
    ...
    cx.on_action(|_: &tau_actions::OpenLicenses, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
    ...
    .on_action(|&tau_actions::OpenKeymapFile, cx| {
    ...
    .on_action(|_: &tau_actions::OpenDefaultKeymap, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may be due to system-wide limits on inotify instances. For troubleshooting see: https://tau.ai/docs
            "#},
            e
        );
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://tau.ai/docs/#could-not-start-inotify");
                    cx.quit();
                });
            }
        })
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may occur on network filesystems and WSL paths. For troubleshooting see: https://tau.ai/docs
            "#},
            e
        );
        let prompt = window.prompt(
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("TAU_ALLOW_EMULATED_GPU").is_err() {
        let (graphics_api, docs_url, open_url) = if cfg!(target_os = "windows") {
            ...
            (
                "Vulkan",
                "https://tau.ai/docs",
                "https://tau.ai/docs#tau-fails-to-open-windows",
            )
        };
        let message = format!(
            db::indoc! {r#"
            TAU uses {} for rendering and requires a compatible GPU.

            Currently you are using a software emulated GPU ({}) which
            will result in awful performance.

            For troubleshooting see: {}
            Set TAU_ALLOW_EMULATED_GPU=1 env var to permanently override.
            "#},
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|workspace, action: &tau_actions::OpenRemote, window, cx| {
        ...
        .register_action({
            ...
            move |_, action: &tau_actions::IncreaseUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &tau_actions::DecreaseUiFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let ui_font_size = ThemeSettings::get_global(cx).ui_font_size(cx) - px(1.0);
                        let _ = settings
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &tau_actions::ResetUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &tau_actions::IncreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &tau_actions::DecreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let buffer_font_size =
                            ThemeSettings::get_global(cx).buffer_font_size(cx) - px(1.0);
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &tau_actions::ResetBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &tau_actions::ResetAllZoom, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
                        settings.theme.ui_font_size = None;
                        settings.theme.buffer_font_size = None;
        ...
        .register_action(|_, _: &install_cli::RegisterTauScheme, window, cx| {
            cx.spawn_in(window, async move |workspace, cx| {
                install_cli::register_tau_scheme(cx).await?;
                workspace.update_in(cx, |workspace, _, cx| {
                    struct RegisterTauScheme;

                    workspace.show_toast(
                        Toast::new(
                            NotificationId::unique::<RegisterTauScheme>(),
                            format!(
                                "tau:// links will now open in {}.",
        ...
        .register_action(
            |workspace: &mut Workspace,
             _: &tau_actions::project_panel::ToggleFocus,
             window: &mut Window,
             cx: &mut Context<Workspace>| {
                        ...
                        MessageNotification::new(format!("Invalid user settings file\n{error}"), cx)
                            ...
                            .primary_on_click(|window, cx| {
                                window.dispatch_action(
                                    tau_actions::OpenSettingsFile.boxed_clone(),
                                    cx,
                                );
            ...
            if !showed_parse_error {
                show_app_notification(id, cx, move |cx| {
                    cx.new(|cx| {
                        ...
                        .primary_icon(IconName::Settings)
                        .primary_on_click(|window, cx| {
                            window.dispatch_action(tau_actions::OpenSettingsFile.boxed_clone(), cx);
                            cx.emit(DismissEvent);
                        })
                    })
                });
...
/// Starts watching `~/.config/tau/AGENTS.md` (or the platform equivalent) and
    ...
    show_app_notification(notification_id, cx, move |cx| {
        cx.new(|cx| {
            MessageNotification::new(message.clone(), cx)
                ...
                .primary_icon(IconName::Settings)
                .primary_on_click(|window, cx| {
                    window.dispatch_action(tau_actions::OpenKeymapFile.boxed_clone(), cx);
                    cx.emit(DismissEvent);
                })
        })
    });
...
fn open_project_debug_tasks_file(
    workspace: &mut Workspace,
    _: &tau_actions::OpenProjectDebugTasks,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    open_local_file(
...
mod tests {
    ...
    fn init_test_with_state(
        ...
        cx.update(move |cx| {
            ...
            web_search::init(cx);
            web_search_providers::init(app_state.client.http_client(), cx);
    ...
    async fn test_opening_project_settings_when_excluded(cx: &mut gpui::TestAppContext) {
        ...
        eprintln!("Created project with .tau/settings.json containing UNIQUEVALUE");
        ...
        let original_settings = app_state
            .fs
            .load(Path::new("/root/.tau/settings.json"))
            .await
            .unwrap();
        ...
        // 3. Add .tau to file scan exclusions in user settings
        cx.update_global::<SettingsStore, _>(|store, cx| {
            store.update_user_settings(cx, |worktree_settings| {
                worktree_settings.project.worktree.file_scan_exclusions =
                    Some(vec![".tau".to_string()]);
            });
        });

        eprintln!("Added .tau to file_scan_exclusions in settings");

        // 4. Run tasks to apply settings
        cx.background_executor.run_until_parked();

        // 5. Critical: Verify .tau is actually excluded from worktree
        let worktree = cx.update(|cx| project.read(cx).worktrees(cx).next().unwrap());

        let has_zed_entry =
            cx.update(|cx| worktree.read(cx).entry_for_path(rel_path(".tau")).is_some());

        eprintln!(
            "Is .tau directory visible in worktree after exclusion: {}",
            has_zed_entry
        );

        // This assertion verifies the test is set up correctly to show the bug
        // If .tau is not excluded, the test will fail here
        assert!(
            !has_zed_entry,
            "Test precondition failed: .tau directory should be excluded but was found in worktree"
        );
