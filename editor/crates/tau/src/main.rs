...
fn build_application() -> Application {
    ...
    if std::env::var("TAU_EXPERIMENTAL_A11Y").as_deref() == Ok("1") {
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "TAU failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "dev.tau.Oops";
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("TAU failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://tau.ai/docs for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
fn main() {
    ...
    // `tau --printenv` Outputs environment variables as JSON to stdout
    if args.printenv {
    ...
    if args.system_specs {
        ...
        println!("TAU System Specs (from CLI):\n{}", system_specs);
        return;
    }
        ...
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(target_os = "windows")]
        {
            !crate::app::windows_only_instance::handle_single_instance(open_listener.clone(), &args)
        }
    ...
    app.run(move |cx| {
        ...
        app::watch_settings_files(fs.clone(), cx);
        ...
        app::watch_user_agents_md(app_state.fs.clone(), cx);
...
fn handle_open_request(request: OpenRequest, app_state: Arc<AppState>, cx: &mut App) {
    if let Some(kind) = request.kind {
        match kind {
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| {
                        window.dispatch_action(
                            Box::new(tau_actions::Extensions {
                    ...
                    multi_workspace.update(cx, |multi_workspace, window, cx| {
                        multi_workspace.workspace().update(cx, |workspace, cx| {
                            if let Some(panel) = workspace.focus_panel::<AgentPanel>(window, cx) {
                                ...
                                log::warn!(
                                    "tau://agent received but the AgentPanel is not registered \
                ...
                workspace::with_active_or_new_workspace(cx, |_workspace, window, cx| {
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let uri = format!("tau://schemas/{}", schema_path);
            ...
            OpenRequestKind::Setting { setting_path } => {
                // tau://settings/languages/$(language)/tab_size  - DONT SUPPORT
                // tau://settings/languages/Rust/tab_size  - SUPPORT
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| match setting_path {
                        None => window.dispatch_action(Box::new(tau_actions::OpenSettings), cx),
                        Some(setting_path) => window.dispatch_action(
                            Box::new(tau_actions::OpenSettingsAt {
                                path: setting_path,
                                target: None,
                            }),
                            cx,
            ...
            OpenRequestKind::GitCommit { sha } => {
                let base_open_options = app::open_options_for_request(
    ...
    if let Some(connection_options) = request.remote_connection {
        ...
        let base_open_options = app::open_options_for_request(open_behavior, &location, cx);
        cx.spawn(async move |cx| {
...
#[derive(Parser, Debug)]
#[command(name = "tau", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `tau://` scheme, or relative to <https://tau.ai>.
    ...
    /// On macOS, the default is `~/Library/Application Support/TAU`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/tau`.
    /// On Windows, the default is `%LOCALAPPDATA%\TAU`.
    ...
    /// prevents TAU from starting, so you can't run `tau: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used for recording minidumps on crashes by having TAU run a separate
    ...
    /// The PID of the TAU process to trace for heap analysis.
    #[cfg(target_os = "windows")]
    ...
    /// Unix socket path for IPC with the parent TAU process.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
    ...
    match std::fs::canonicalize(Path::new(&arg)) {
        ...
        Err(_) => {
            if arg.starts_with("file://")
                || arg.starts_with("tau://")
                || arg.starts_with("tau-cli://")
                || arg.starts_with("ssh://")
                || parse_tau_link(arg, cx).is_some()
            {
                arg.into()
            } else {
                format!("file://{arg}")
