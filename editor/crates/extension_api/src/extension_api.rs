//! The Tau Rust Extension API allows you write extensions for [Tau](https://tau.dev/) in Rust.
...
pub use wit::{
    ...
    tau::extension::context_server::ContextServerConfiguration,
    tau::extension::dap::{
    ...
    tau::extension::github::{
        GithubRelease, GithubReleaseAsset, GithubReleaseOptions, github_release_by_tag_name,
        latest_github_release,
    },
    tau::extension::nodejs::{
        node_binary_path, npm_install_package, npm_package_installed_version,
        npm_package_latest_version,
    },
    tau::extension::platform::{Architecture, Os, current_platform},
    tau::extension::slash_command::{
...
pub mod lsp {
    pub use crate::wit::tau::extension::lsp::{
        Completion, CompletionKind, InsertTextFormat, Symbol, SymbolKind,
    };
}

/// A result returned from a Tau extension.
...
pub trait Extension: Send + Sync {
    ...
    /// Locators are entities that convert a Tau task into a debug scenario.
    ///
    /// They can be provided even by extensions that don't provide a debug adapter.
    /// For all tasks applicable to a given buffer, Tau will query all locators to find one that can turn the task into a debug scenario.
    ...
    /// `dap_locator_create_scenario` if you're able to do that. Make sure to not fill out `build` field in that case, as that will prevent Tau from running second phase of resolution in such case.
...
/// Registers the provided type as a Tau extension.
///
...
#[unsafe(link_section = "tau:api-version")]
#[doc(hidden)]
pub static TAU_API_VERSION: [u8; 6] = *include_bytes!(concat!(env!("OUT_DIR"), "/version_bytes"));
