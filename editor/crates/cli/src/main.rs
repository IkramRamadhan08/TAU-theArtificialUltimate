...
const URL_PREFIX: [&'static str; 5] = ["tau://", "http://", "https://", "file://", "ssh://"];
...
#[command(
    ...
    before_help = "The TAU CLI binary.
This CLI is a separate binary that invokes TAU.
...
Examples:
    ...
    `tau path-to-your-project`
          Open your project in TAU
    `tau -n path-to-file `
          Open file/folder in a new window",
    after_help = "To read from stdin, append '-', e.g. 'ps axf | tau -'"
)]
struct Args {
    ...
    #[cfg_attr(target_os = "macos", doc = "`~/Library/Application Support/TAU`.")]
    #[cfg_attr(target_os = "windows", doc = "`%LOCALAPPDATA%\\TAU`.")]
    #[cfg_attr(
        not(any(target_os = "windows", target_os = "macos")),
        doc = "`$XDG_DATA_HOME/tau`."
    )]
    #[arg(long, value_name = "DIR")]
    user_data_dir: Option<String>,
    /// The paths to open in TAU (space-separated).
    ///
    /// Use `path:line:column` syntax to open a file at the given line and column.
    paths_with_position: Vec<String>,
    /// Print TAU's version and the app path.
    #[arg(short, long)]
    version: bool,
    /// Run TAU in the foreground (useful for debugging)
    #[arg(long)]
    foreground: bool,
    /// Custom path to TAU.app or the tau binary
    #[arg(long)]
    tau: Option<PathBuf>,
    /// Run tau in dev-server mode
    #[arg(long)]
    dev_server_token: Option<String>,
    /// The username and WSL distribution to use when opening paths. If not specified,
    /// TAU will attempt to open the paths directly.
    ///
    ...
    wsl: Option<String>,
    /// Not supported in TAU CLI, only supported on TAU binary
    ...
    diff: Vec<String>,
    /// Uninstall TAU from user system
    #[cfg(all(
    ...
    /// by having TAU act like netcat communicating over a Unix socket.
    #[arg(long, hide = true)]
...
///
/// This method must return an absolute path, as many TAU
...
fn run() -> Result<()> {
    ...
    if let Ok(socket) = std::env::var("TAU_ASKPASS_SOCKET") {
    ...
    let app = Detect::detect(args.tau.as_deref()).context("Bundle detection")?;
    ...
    if args.system_specs {
        let path = app.path();
        let msg = [
            "The `--system-specs` argument is not supported in the TAU CLI, only on the TAU binary.",
    ...
    if args.uninstall {
        ...
        let status = std::process::Command::new("sh")
            .arg(&script_path)
            .env("TAU_CHANNEL", &*release_channel::RELEASE_CHANNEL_NAME)
            .status()
    ...
    let (server, server_name) =
        IpcOneShotServer::<IpcHandshake>::new().context("Handshake before Tau spawn")?;
    let url = format!("tau-cli://{server_name}");
    ...
    let env = {
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            ...
            // On Linux, the desktop entry uses `cli` to spawn `tau`.
    ...
    // for directory diffs. The CLI process may exit before Tau has read these
    ...
    anyhow::ensure!(
        ...
        "Dev servers were removed in v0.157.x please upgrade to SSH remoting: https://tau.dev/docs/remote-development"
    );
    ...
    let sender: JoinHandle<anyhow::Result<()>> = thread::Builder::new()
        ...
        .spawn({
            ...
            move || {
                let (_, handshake) = server.accept().context("Handshake after Tau spawn")?;
...
/// behavior for `tau <path>`. Returns `None` if the prompt cannot be shown
...
fn prompt_open_behavior() -> Option<cli::CliBehaviorSetting> {
    ...
    let items = [
        format!(
            "Add to existing TAU window ({})",
            blue.apply_to("tau --existing")
        ),
        format!("Open a new window ({})", blue.apply_to("tau --classic")),
    ];

    let prompt = format!(
        "Configure default behavior for {}\n{}",
        blue.apply_to("tau <path>"),
        console::style("You can change this later in TAU settings"),
    );
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // libexec is the standard, lib/tau is for non-libexec distros,
                // ./tau is for the target directory in development builds.
                let possible_locations =
                    ["../libexec/tau-editor", "../lib/tau/tau-editor", "./tau"];
                possible_locations
                    .iter()
    ...
    impl InstalledApp for App {
        fn zed_version_string(&self) -> String {
            format!(
                ...
                match option_env!("TAU_COMMIT_SHA") {
...
mod flatpak {
    ...
    const EXTRA_LIB_ENV_NAME: &str = "TAU_FLATPAK_LIB_PATH";
    const NO_ESCAPE_ENV_NAME: &str = "TAU_FLATPAK_NO_ESCAPE";
    ...
    pub fn try_restart_to_host() {
        if let Some(flatpak_dir) = get_flatpak_dir() {
            ...
            args.push("--env=TAU_UPDATE_EXPLANATION=Please use flatpak to update tau".into());
            args.push(
                format!(
            ...
            args.push(flatpak_dir.join("bin").join("tau").into());

            let mut is_app_location_set = false;
            for arg in &env::args_os().collect::<Vec<_>>()[1..] {
                args.push(arg.clone());
                is_app_location_set |= arg == "--tau";
            }

            if !is_app_location_set {
                args.push("--tau".into());
                args.push(flatpak_dir.join("libexec").join("tau-editor").into());
            }
    ...
    pub fn set_bin_if_no_escape(mut args: super::Args) -> super::Args {
        if env::var(NO_ESCAPE_ENV_NAME).is_ok()
            && env::var("FLATPAK_ID").is_ok_and(|id| id.starts_with("ai.tau.TAU"))
            && args.tau.is_none()
        {
            args.tau = Some("/app/libexec/tau-editor".into());
            unsafe { env::set_var("TAU_UPDATE_EXPLANATION", "Please use flatpak to update TAU") };
        }
        args
    }
    ...
    fn get_flatpak_dir() -> Option<PathBuf> {
        ...
        if let Ok(flatpak_id) = env::var("FLATPAK_ID") {
            if !flatpak_id.starts_with("ai.tau.TAU") {
                return None;
            }
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
        fn zed_version_string(&self) -> String {
            format!("TAU {} - {}", self.version(), self.path().display(),)
        }
            ...
            match self {
                Self::App { app_bundle, .. } => {
                    ...
                    let status = unsafe {
                        ...
                        // equivalent to: open tau-cli:... -a /Applications/TAU\ Preview.app
                        let urls_to_open =
                ...
                Self::LocalPath { executable, .. } => {
                    ...
                    let subprocess_stdout_file = fs::File::create(
                        executable_parent.join("tau_dev.log"),
                    )
        ...
        fn run_foreground(
            ...
            let path = match self {
                Bundle::App { app_bundle, .. } => app_bundle.join("Contents/MacOS/tau"),
