...
static TAU_CLIENT_CHECKSUM_SEED: LazyLock<Option<Vec<u8>>> = LazyLock::new(|| {
    option_env!("TAU_CLIENT_CHECKSUM_SEED")
        .map(|s| s.as_bytes().into())
        .or_else(|| {
            env::var("TAU_CLIENT_CHECKSUM_SEED")
                .ok()
                .map(|s| s.as_bytes().into())
        })
});
...
pub static MINIDUMP_ENDPOINT: LazyLock<Option<String>> = LazyLock::new(|| {
    option_env!("TAU_MINIDUMP_ENDPOINT")
        .map(str::to_string)
        .or_else(|| env::var("TAU_MINIDUMP_ENDPOINT").ok())
});
...
impl Telemetry {
    ...
    pub fn has_checksum_seed(&self) -> bool {
        TAU_CLIENT_CHECKSUM_SEED.is_some()
    }
    ...
    fn report_event(self: &Arc<Self>, mut event: Event) {
        ...
        match &mut event {
            Event::Flexible(event) => event
                .event_properties
                .insert("event_source".into(), "tau".into()),
        };
    ...
    fn build_request(
        ...
        Ok(Request::builder()
            ...
            .header("x-tau-checksum", checksum)
...
pub fn calculate_json_checksum(json: &impl AsRef<[u8]>) -> Option<String> {
    let checksum_seed = TAU_CLIENT_CHECKSUM_SEED.as_ref()?;
