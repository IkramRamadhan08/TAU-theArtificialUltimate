//! Contains helper functions for constructing URLs to various Tau-related pages.
//!
...
/// Returns the URL to the account page on tau.ai.
pub fn account_url(cx: &App) -> String {
    format!("{server_url}/account", server_url = server_url(cx))
}

/// Returns the URL to the start trial page on tau.ai.
...
/// Returns the URL to Tau's edit prediction documentation.
...
/// Returns the URL to Tau's ACP registry blog post.
...
pub fn shared_agent_thread_url(session_id: &str) -> String {
    format!("tau://agent/shared/{}", session_id)
}
