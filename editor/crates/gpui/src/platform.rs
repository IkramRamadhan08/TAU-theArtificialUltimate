...
pub fn guess_compositor() -> &'static str {
    if std::env::var_os("TAU_HEADLESS").is_some() {
        return "Headless";
    }
...
impl WindowButtonLayout {
    /// Returns Tau's built-in fallback button layout for Linux titlebars.
...
/// Tau's interface for handling text input from the platform's IME system
