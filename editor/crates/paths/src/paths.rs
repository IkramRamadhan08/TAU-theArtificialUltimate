//! Paths to locations used by TAU.
...
///
/// Forks should change this to avoid colliding with upstream user data.
pub const APP_NAME: &str = "TAU";
...
/// On macOS, this is `~/Library/Application Support/TAU`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/tau`.
/// On Windows, this is `%LOCALAPPDATA%\TAU`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/tau`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/tau`.
/// On Windows, this is `%APPDATA%\TAU`.
...
/// Returns the path to the configuration directory used by TAU.
...
/// Returns the path to the data directory used by TAU.
...
/// Returns the path to the temp directory used by TAU.
...
/// Returns the path to the TAU server state directory on this SSH host.
...
/// Returns the path to the `TAU.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join(format!("{}.log", APP_NAME)))
}

/// Returns the path to the `TAU.log.old` file.
...
/// user opens, and is loaded into the native TAU agent's system prompt.
...
/// # Arguments
///
/// * `dev_mode` - If true, assumes the current working directory is the Tau repository.
...
///
/// This is where language servers are downloaded to for languages built-in to Tau.
...
///
/// This is where debug adapters are downloaded to for DAPs that are built-in to Tau.
...
/// Returns the relative path to a `.tau` folder within a project.
...
pub fn local_settings_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".tau/settings.json").unwrap());
    *CACHED
}
...
pub fn local_tasks_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".tau/tasks.json").unwrap());
    *CACHED
}
...
pub fn local_debug_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".tau/debug.json").unwrap());
    *CACHED
}
