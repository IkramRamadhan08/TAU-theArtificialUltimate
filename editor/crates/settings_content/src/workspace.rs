...
pub struct WorkspaceSettingsContent {
    ...
    /// Controls previous session restoration in freshly launched Tau instance.
    ...
    /// When set to false, Tau will use the built-in keyboard-first pickers.
    ///
    /// Default: true
    pub use_system_path_prompts: Option<bool>,
    /// Whether to use the system provided prompts.
    /// When set to false, Tau will use the built-in prompts.
    /// Note that this setting has no effect on Linux, where Tau will always
    ...
    /// What draws window decorations/titlebar, the client application (Tau) or display server
    /// Default: client
...
pub enum WindowDecorations {
    /// Tau draws its own window decorations/titlebar (client-side decoration)
    #[default]
    Client,
...
pub enum CliDefaultOpenBehavior {
    /// Open directories as a new workspace in the current Tau window's sidebar.
    #[default]
...
pub enum DefaultOpenBehavior {
    /// Open projects in the current Tau window.
    #[default]
    #[strum(serialize = "Add to Existing Window")]
    ExistingWindow,
...
pub enum RestoreOnStartupBehavior {
    ...
    LastWorkspace,
    /// Restore all workspaces that were open when quitting Tau.
    #[default]
    LastSession,
