...
use project::{
    Project, RevealStrategy, RevealTarget, TaskContext, TaskSourceKind, TaskTemplate,
    git_store::GitStoreEvent, project_settings::ProjectSettings, trusted_worktrees::TrustedWorktrees,
};
                ...
                .map(|title_bar| {
                    ...
                    title_bar
                        ...
                        .when(render_project_items, |title_bar| {
                            title_bar
                                .child(self.render_run_button(window, cx))
...
impl TitleBar {
    pub fn new(
        ...
        let application_menu = match platform_style {
            PlatformStyle::Mac => {
                if option_env!("TAU_USE_CROSS_PLATFORM_MENU").is_some() {
    ...
    fn render_project_name(
        ...
        PopoverMenu::new("recent-projects-menu")
            ...
            .trigger_with_tooltip(
                ...
                move |_window, cx| {
                    Tooltip::for_action("Recent Projects", &tau_actions::OpenRecent::default(), cx)
                },
            )
            .anchor(gpui::Anchor::TopLeft)
            .into_any_element()
    ...
    fn render_run_button(&self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let workspace = self.workspace.clone();
        let cmd = run_command.clone();
        let tooltip_text: SharedString = run_command.as_ref()
            .map(|(c, _)| format!("Run: {}", c).into())
            .unwrap_or_else(|| "No project detected".into());

        Button::new("run-button", "▶ Run")
            .on_click(move |_, window, cx| {
                let Some(workspace) = workspace.upgrade() else { return };
                let Some((ref command, ref label)) = cmd else { return };
                let cwd = {
                    let project = workspace.read(cx).project().clone();
                    project.read(cx).visible_worktrees(cx).next()
                ...
                let task_cx = TaskContext {
                    cwd: task.cwd.as_ref().map(|p| std::path::PathBuf::from(p)),
                    ..TaskContext::default()
                };
    ...
    fn detect_run_command(&self, cx: &mut Context<Self>) -> Option<(String, String)> {
        let worktree = self.project.read(cx).visible_worktrees(cx).next()?;
        let path = worktree.read(cx).abs_path();

        let entries = [
            ("Cargo.toml", "cargo run", "Cargo Run"),
            ("package.json", "npm start", "npm start"),
            ("index.html", "xdg-open index.html", "Open Browser"),
            ("main.py", "python3 main.py", "Python Run"),
            ("app.py", "python3 app.py", "Python Run"),
            ("go.mod", "go run .", "Go Run"),
            ("Makefile", "make", "Make Build"),
            ("makefile", "make", "Make Build"),
            ("Dockerfile", "docker build .", "Docker Build"),
            ("docker-compose.yml", "docker compose up", "Docker Compose"),
            ("composer.json", "php -S localhost:8000", "PHP Serve"),
            ("pom.xml", "mvn compile exec:java", "Maven Run"),
            ("Gemfile", "bundle exec ruby main.rb", "Ruby Run"),
        ];

        for (filename, command, label) in &entries {
            if path.join(filename).exists() {
                if *filename == "package.json" {
                    // Check for npm run dev script via simple string search
                    if let Ok(content) = std::fs::read_to_string(path.join("package.json")) {
                        if content.contains("\"dev\"") || content.contains("'dev'") {
                            return Some(("npm run dev".into(), "npm run dev".into()));
                        }
                    }
                }
                return Some((command.to_string(), label.to_string()));
            }
        }

        // Check for .csproj files
        if std::fs::read_dir(&path).ok()?.any(|e| {
            e.ok().map_or(false, |entry| entry.path().extension() == Some(std::ffi::OsStr::new("csproj")))
        }) {
            return Some(("dotnet run".into(), "dotnet Run".into()));
        }

        // Check for index.js / app.js as last resort
        if path.join("index.js").exists() {
            return Some(("node index.js".into(), "Node Run".into()));
        }
        if path.join("app.js").exists() {
            return Some(("node app.js".into(), "Node Run".into()));
        }
        ...
        match status {
            ...
            client::Status::UpgradeRequired => {
                ...
                let label = match auto_updater.map(|auto_update| auto_update.read(cx).status()) {
                    Some(AutoUpdateStatus::Updated { .. }) => "Please restart TAU to Collaborate",
                    Some(AutoUpdateStatus::Installing { .. })
                    | Some(AutoUpdateStatus::Downloading { .. })
                    | Some(AutoUpdateStatus::Checking) => "Updating...",
                    Some(AutoUpdateStatus::Idle)
                    | Some(AutoUpdateStatus::Errored { .. })
                    | None => "Please update TAU to Collaborate",
                };
        ...
        PopoverMenu::new("user-menu")
            ...
            .menu(move |window, cx| {
                ...
                ContextMenu::build(window, cx, |menu, _, _cx| {
                        ...
                        this.custom_entry(
                            move |_window, _cx| {
                                h_flex()
                                    ...
                                    .gap_1()
                                    .justify_between()
                                    .child(Label::new("Restart to update TAU").color(Color::Accent))
                                    .child(
                    ...
                    .action("Settings", tau_actions::OpenSettings.boxed_clone())
                    .action("Keymap", Box::new(tau_actions::OpenKeymap))
                    .action(
                        "Themes…",
                        tau_actions::theme_selector::Toggle::default().boxed_clone(),
                    )
                    .action(
                        "Icon Themes…",
                        tau_actions::icon_theme_selector::Toggle::default().boxed_clone(),
                    )
                    .action(
                        "Extensions",
                        tau_actions::Extensions::default().boxed_clone(),
                    )
