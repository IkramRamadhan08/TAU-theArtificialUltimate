...
/// completion popup. Only the native TAU agent annotates its commands; commands
/// from external ACP agents carry no category and are grouped on their own.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CommandCategory {
    /// Built-in TAU agent commands (e.g. `/compact`).
    Native,
...
impl TokenUsage {
    pub fn ratio(&self) -> TokenUsageRatio {
        ...
        let warning_threshold: f32 = std::env::var("TAU_THREAD_WARNING_THRESHOLD")
...
mod tests {
    ...
    async fn test_thinking_concatenation(cx: &mut gpui::TestAppContext) {
        ...
        thread
            .update(cx, |thread, cx| thread.send_raw("Hello from Tau!", cx))
            .await
            .unwrap();
    ...
    async fn test_ignore_echoed_user_message_chunks_during_active_turn(
        ...
        let output = thread.read_with(cx, |thread, cx| thread.to_markdown(cx));
        assert_eq!(output.matches("Hello from Tau!").count(), 1);
    }
    ...
    /// loading state until Tau restarts.
    #[gpui::test]
