...
//!   (still using Tau's shipped default content) are skipped so we don't
...
//! downgrades to a Tau build without skills support won't lose anything.
...
///    built-ins are skipped so we don't write Tau's shipped default text
