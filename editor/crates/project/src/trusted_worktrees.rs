//! A module, responsible for managing the trust logic in Tau.
//!
...
//!
//! Tau does not consider invisible, `worktree.is_visible() == false` worktrees in Tau, as those are programmatically created inside Tau for internal needs, e.g. a tmp dir for `keymap_editor.rs` needs.
//!
//!
//! Path rust hierarchy.
//!
//! Tau has multiple layers of trust, based on the requests and [`PathTrust`] enum variants.
//! From the least to the most trusted level:
//!
//! * "single file worktree"
//!
//! After opening an empty Tau it's possible to open just a file, same as after opening a directory in Tau it's possible to open a file outside of this directory.
//! Usual scenario for both cases is opening Tau's settings.json file via `tau: open settings file` command: that starts a language server for a new file open, which originates from a newly created, single file worktree.
//!
//! Spawning a language server is potentially dangerous, and Tau needs to restrict that by default.
//! Each single file worktree requires a separate trust permission, unless a more global level is trusted.
//!
//! * "directory worktree"
//!
//! If a directory is open in Tau, it's a full worktree which may spawn multiple language servers associated with it.
...
//! * "path override"
//!
//! To ease trusting multiple directory worktrees at once, it's possible to trust a parent directory of a certain directory worktree opened in Tau.
...
impl TrustedWorktreesStore {
    ...
    /// Requires Tau's restart to take proper effect.
    ...
    pub fn can_trust(
        ...
        // Tau opened an "internal" directory: e.g. a tmp dir for `keymap_editor.rs` needs.
