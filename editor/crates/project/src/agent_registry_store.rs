...
impl AgentRegistryStore {
    ...
    ///
    /// This loads the cached registry from disk but does NOT trigger a network fetch
    /// on startup (TAU: registry disabled by default). External agents must be
    /// configured manually via settings.
    /// Call `refresh()` explicitly when you need fresh data.
    pub fn init_global(
        cx: &mut App,
        fs: Arc<dyn Fs>,
        http_client: Arc<dyn HttpClient>,
