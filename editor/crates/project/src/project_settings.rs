...
pub struct NodeBinarySettings {
    ...
    /// The path to the npm binary Tau should use (defaults to `.path/../npm`).
    pub npm_path: Option<String>,
    /// If enabled, Tau will download its own copy of Node.
...
pub struct DiagnosticsSettings {
    ...
    /// Settings for using LSP pull diagnostics mechanism in Tau.
...
/// SettingsObserver observers changes to .tau/{settings, task}.json files in local worktrees
/// (or the equivalent protobuf messages from upstream) and updates local settings
/// and sends notifications downstream.
/// In ssh mode it also monitors ~/.config/tau/{settings, task}.json and sends the content
/// upstream.
                            ...
                            async move {
                                ...
                                if abs_path.ends_with(local_vscode_tasks_file_relative_path().as_std_path()) {
                                    ...
                                    let zed_tasks = TaskTemplates::try_from(vscode_tasks)
                                        .with_context(|| {
                                            format!(
                                        "converting VSCode tasks into Tau ones, file {abs_path:?}"
                                    )
                                        })?;
                                    serde_json::to_string(&zed_tasks).with_context(|| {
                                        format!(
                                            "serializing Tau tasks into JSON, file {abs_path:?}"
                                        )
                                    })
                                    ...
                                    let zed_tasks = DebugTaskFile::try_from(vscode_tasks)
                                        .with_context(|| {
                                            format!(
                                        "converting VSCode debug tasks into Tau ones, file {abs_path:?}"
                                    )
                                        })?;
                                    serde_json::to_string(&zed_tasks).with_context(|| {
                                        format!(
