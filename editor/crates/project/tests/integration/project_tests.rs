...
async fn test_editorconfig_support(cx: &mut gpui::TestAppContext) {
    ...
    // .editorconfig overrides .tau/settings
    ...
    // When max_line_length is "off", default to .tau/settings.json
...
async fn test_git_provider_project_setting(cx: &mut gpui::TestAppContext) {
    ...
    fs.atomic_write(
        Path::new(path!("/dir/.tau/settings.json")).to_owned(),
        "{}".into(),
    )
    .await
    .unwrap();
...
async fn test_managing_project_specific_settings(cx: &mut gpui::TestAppContext) {
    ...
    let topmost_local_task_source_kind = TaskSourceKind::Worktree {
        id: worktree_id,
        directory_in_worktree: rel_path(".tau").into(),
        id_base: "local worktree tasks from directory \".tau\"".into(),
    };
        ...
        vec![
            (
                TaskSourceKind::Worktree {
                    id: worktree_id,
                    directory_in_worktree: rel_path("b/.tau").into(),
                    id_base: "local worktree tasks from directory \"b/.tau\"".into()
                },
...
async fn test_invalid_local_tasks_shows_toast_with_doc_link(cx: &mut gpui::TestAppContext) {
    ...
    // We need to start with a valid `.tau/tasks.json` file as otherwise the
    ...
    // Update the `.tau/tasks.json` file with an invalid variable, so we can
    // later assert that the `Event::Toast` even is emitted.
    fs.save(
        path!("/dir/.tau/tasks.json").as_ref(),
        &r#"[{ "label": "test $TAU_FOO", "command": "echo" }]"#.into(),
        Default::default(),
    )
    .await
    .unwrap();
    ...
    project.update(cx, |_, cx| {
        ...
        cx.subscribe(&project, move |_, _, event: &Event, _| match event {
            Event::Toast {
                ...
                assert!(message.contains("TAU_FOO"));
                assert_eq!(*url, "https://tau.dev/docs/tasks");
...
async fn test_fallback_to_single_worktree_tasks(cx: &mut gpui::TestAppContext) {
        ...
        json!({
            ".tau": {
                "tasks.json": r#"[{
                    ...
                    "command": "echo $TAU_WORKTREE_ROOT"
                }]"#,
            },
            "a": {
                "a.rs": "fn a() {\n    A\n}"
    ...
    assert!(
        ...
        "A task can not be resolved with context with no TAU_WORKTREE_ROOT data"
    );
