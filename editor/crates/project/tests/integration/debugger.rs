mod go_locator {
    ...
    async fn test_create_scenario_for_go_build(_: &mut TestAppContext) {
        ...
        let task = TaskTemplate {
            ...
            env: Default::default(),
            cwd: Some("${TAU_WORKTREE_ROOT}".into()),
...
mod python_locator {
    ...
    async fn test_python_locator() {
        ...
        let build_task = TaskTemplate {
            label: "run module '$TAU_FILE'".into(),
            command: "$TAU_CUSTOM_PYTHON_ACTIVE_TAU_TOOLCHAIN".into(),
            args: vec!["-m".into(), "$TAU_CUSTOM_PYTHON_MODULE_NAME".into()],
            env: Default::default(),
            cwd: Some("$TAU_WORKTREE_ROOT".into()),
            use_new_terminal: false,
            allow_concurrent_runs: false,
        ...
        let expected_scenario = DebugScenario {
            ...
            config: json!({
                "request": "launch",
                "python": "$TAU_CUSTOM_PYTHON_ACTIVE_TAU_TOOLCHAIN",
                "args": [],
                "cwd": "$TAU_WORKTREE_ROOT",
                "module": "$TAU_CUSTOM_PYTHON_MODULE_NAME",
            }),
            tcp_connection: None,
        };
