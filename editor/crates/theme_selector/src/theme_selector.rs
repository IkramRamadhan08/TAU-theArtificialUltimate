...
use tau_actions::{ExtensionCategoryFilter, Extensions};
...
pub fn init(cx: &mut App) {
    cx.on_action(|action: &tau_actions::theme_selector::Toggle, cx| {
        let action = action.clone();
        with_active_or_new_workspace(cx, move |workspace, window, cx| {
            toggle_theme_selector(workspace, &action, window, cx);
        });
    });
    cx.on_action(|action: &tau_actions::icon_theme_selector::Toggle, cx| {
        let action = action.clone();
        with_active_or_new_workspace(cx, move |workspace, window, cx| {
...
fn toggle_theme_selector(
    ...
    toggle: &tau_actions::theme_selector::Toggle,
    window: &mut Window,
...
fn toggle_icon_theme_selector(
    workspace: &mut Workspace,
    toggle: &tau_actions::icon_theme_selector::Toggle,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    let fs = workspace.app_state().fs.clone();
                ...
                .child(
                    Button::new("docs", "View Theme Docs")
                        ...
                        .on_click(cx.listener(|_, _, _, cx| {
                            cx.open_url("https://tau.dev/docs/themes");
                        })),
                )
                .child(
...
mod tests {
    ...
    fn open_theme_selector(
        ...
        cx.dispatch_action(tau_actions::theme_selector::Toggle {
            themes_filter: None,
        });
        cx.run_until_parked();
