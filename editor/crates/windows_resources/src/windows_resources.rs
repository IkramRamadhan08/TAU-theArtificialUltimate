...
fn git_sha() -> Option<String> {
    if let Ok(sha) = std::env::var("TAU_COMMIT_SHA") {
        return Some(sha);
    }
...
const ICON_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../tau/resources/windows");
...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "TAU"),
        "preview" => ("app-icon-preview.ico", "TAU Preview"),
        "nightly" => ("app-icon-nightly.ico", "TAU Nightly"),
        _ => ("app-icon-dev.ico", "TAU Dev"),
    };
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "CompanyName", "TAU\0"
            VALUE "LegalCopyright", "Copyright 2026 TAU contributors\0"
        END
    END
    BLOCK "VarFileInfo"
    BEGIN
...
"#
    ...
    if let Ok(toolkit_path) = std::env::var("TAU_RC_TOOLKIT_PATH") {
