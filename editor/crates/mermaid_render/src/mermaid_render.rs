...
//! pipeline. Tau opts into that pipeline during rendering, then keeps
...
//! - [`postprocess`] the SVG to add Tau theme and accent styling.
//!
//! Tau's postprocessing is split up into stages. We parse the generated SVG
...
//! `player_colors` in the Tau theme.
//!
...
//!    `tau-accent-0`) to specific elements, based on the diagram type and
//!    node.
...
/// from Tau's UI palette) round-trip without silently losing their alpha.
