...
impl HttpClientWithUrl {
    ...
    /// Builds a Tau API URL using the given path.
    pub fn build_zed_api_url(&self, path: &str, query: &[(&str, &str)]) -> Result<Url> {
        let base_url = self.base_url();
        let base_api_url = match base_url.as_ref() {
            "https://tau.ai" => "https://api.tau.ai",
            "https://staging.tau.ai" => "https://api-staging.tau.ai",
    ...
    /// Builds a Tau Cloud URL using the given path.
    pub fn build_zed_cloud_url(&self, path: &str) -> Result<Url> {
        let base_url = self.base_url();
        let base_api_url = match base_url.as_ref() {
            "https://tau.ai" => "https://cloud.tau.ai",
            "https://staging.tau.ai" => "https://cloud.tau.ai",
    ...
    /// Builds a Tau Cloud URL using the given path and query params.
    ...
    /// Builds a Tau LLM URL using the given path.
    pub fn build_zed_llm_url(&self, path: &str, query: &[(&str, &str)]) -> Result<Url> {
        let base_url = self.base_url();
        let base_api_url = match base_url.as_ref() {
            "https://tau.ai" => "https://cloud.tau.ai",
            "https://staging.tau.ai" => "https://llm-staging.tau.ai",
            "http://localhost:3000" => "http://localhost:8787",
            other => other,
        };
