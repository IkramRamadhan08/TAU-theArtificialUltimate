...
/// `TAU_ALLOW_ROOT` environment variable.
#[cfg(unix)]
pub fn prevent_root_execution() {
    let is_root = nix::unistd::geteuid().is_root();
    let allow_root = std::env::var("TAU_ALLOW_ROOT").is_ok_and(|val| val == "true");

    if is_root && !allow_root {
        eprintln!(
            "\
Error: Running TAU as root or via sudo is unsupported.
       Doing so (even once) may subtly break things for all subsequent non-root usage of TAU.
       It is untested and not recommended, don't complain when things break.
       If you wish to proceed anyways, set `TAU_ALLOW_ROOT=true` in your environment."
        );
        std::process::exit(1);
    }
}
...
/// Returns a shell escaped path for the current TAU executable
pub fn get_shell_safe_zed_path(shell_kind: shell::ShellKind) -> anyhow::Result<String> {
    use anyhow::Context as _;
    use paths::PathExt;
    let mut zed_path =
        std::env::current_exe().context("Failed to determine current TAU executable path.")?;
    ...
    zed_path
        ...
        .context("Failed to shell-escape TAU executable path.")
}

/// Returns a path for the TAU cli executable, this function
/// should be called from the TAU executable, not the CLI.
pub fn get_zed_cli_path() -> Result<PathBuf> {
    ...
    let parent = zed_path
        .parent()
        .context("Failed to determine parent directory of TAU executable path.")?;

    let possible_locations: &[&str] = if cfg!(target_os = "macos") {
        // On macOS, the TAU executable and CLI are inside the app bundle,
        // so here ./cli is for both installed and development builds.
        &["./cli"]
    } else if cfg!(target_os = "windows") {
        // bin/tau.exe is for installed builds, ./cli.exe is for development builds.
        &["bin/tau.exe", "./cli.exe"]
        ...
        anyhow::bail!("unsupported platform for determining TAU cli path");
    };
    ...
    possible_locations
        ...
        .with_context(|| {
            format!(
                "could not find TAU CLI from any of: {}",
...
pub async fn load_login_shell_environment() -> Result<()> {
    ...
    {
        // Skip SHLVL to prevent it from polluting Tau's process environment.
        // The login shell used for env capture increments SHLVL, and if we propagate it,
        // terminals spawned by Tau will inherit it and increment again, causing SHLVL
