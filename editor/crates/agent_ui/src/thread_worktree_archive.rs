...
/// Resolves the Tau-managed worktrees base directory for a given repo.
///
/// This intentionally reads the *global* `git.worktree_directory` setting
/// rather than any project-local override, because Tau always uses the
...
pub fn build_root_plan(
    ...
    // Only archive worktrees that live inside the Tau-managed worktrees
        ...
        workspace.read_with(cx, |_workspace, cx| {
            ...
            assert!(
                ...
                "build_root_plan should return None for a linked worktree \
                 outside the Tau-managed worktrees directory",
            );
        });
    }
