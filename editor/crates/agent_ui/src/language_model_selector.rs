...
use ui::prelude::*;
use tau_actions::agent::OpenSettings;
...
mod tests {
        ...
        model_specs
            ...
            .map(|(provider, name)| {
                ...
                ModelInfo {
                    ...
                    icon: IconOrSvg::Icon(IconName::TauAgent),
                    is_favorite,
                }
            })
            .collect()
    ...
    fn test_exact_match(cx: &mut TestAppContext) {
        let models = create_models(vec![
            ("tau", "Claude 3.7 Sonnet"),
            ("tau", "Claude 3.7 Sonnet Thinking"),
            ("tau", "gpt-5"),
            ("tau", "gpt-5-mini"),
    ...
    fn test_fuzzy_match(cx: &mut TestAppContext) {
        ...
        // In the case below, `tau/gpt-5-mini` and `openai/gpt-5-mini` have identical
        // similarity scores, but `tau/gpt-5-mini` was higher in the models list,
        // so it should appear first in the results.
        let results = matcher.fuzzy_search("mini");
        assert_models_eq(results, vec!["tau/gpt-5-mini", "openai/gpt-5-mini"]);
        ...
        assert_models_eq(results, vec!["tau/Claude 3.7 Sonnet Thinking"]);
    }

    #[gpui::test]
    fn test_recommended_models_also_appear_in_other(_cx: &mut TestAppContext) {
        let recommended_models = create_models(vec![("tau", "claude")]);
        let all_models = create_models(vec![
            ("tau", "claude"), // Should also appear in "other"
            ("tau", "gemini"),
            ("copilot", "o3"),
        ]);
        ...
        assert_models_eq(
            actual_all_models,
            vec!["tau/claude", "tau/gemini", "copilot/o3"],
        );
    }
    ...
    fn test_models_from_different_providers(_cx: &mut TestAppContext) {
        ...
        assert_models_eq(
            actual_all_models,
            vec!["tau/claude", "tau/gemini", "copilot/claude"],
        );
    }
    ...
    fn test_favorites_section_appears_when_favorites_exist(_cx: &mut TestAppContext) {
        ...
        let all_models = create_models_with_favorites(
            vec![("tau", "claude"), ("tau", "gemini"), ("openai", "gpt-4")],
            vec![("tau", "gemini")],
        );
        ...
        assert_models_eq(grouped_models.favorites, vec!["tau/gemini"]);
    }

    #[gpui::test]
    fn test_no_favorites_section_when_no_favorites(_cx: &mut TestAppContext) {
        let recommended_models = create_models(vec![("tau", "claude")]);
        let all_models = create_models(vec![("tau", "claude"), ("tau", "gemini")]);
    ...
    fn test_models_have_correct_actions(_cx: &mut TestAppContext) {
        let recommended_models =
            create_models_with_favorites(vec![("tau", "claude")], vec![("tau", "claude")]);
        let all_models = create_models_with_favorites(
            vec![("tau", "claude"), ("tau", "gemini"), ("openai", "gpt-4")],
            vec![("tau", "claude")],
        );
        ...
        for entry in &entries {
            if let LanguageModelPickerEntry::Model(info) = entry {
                if info.model.telemetry_id() == "tau/claude" {
                    assert!(info.is_favorite, "tau/claude should be a favorite");
                } else {
                    assert!(
                        !info.is_favorite,
    ...
    fn test_favorites_appear_in_other_sections(_cx: &mut TestAppContext) {
        let favorites = vec![("tau", "gemini"), ("openai", "gpt-4")];

        let recommended_models =
            create_models_with_favorites(vec![("tau", "claude")], favorites.clone());
        ...
        assert_models_eq(grouped_models.favorites, vec!["tau/gemini", "openai/gpt-4"]);
        assert_models_eq(grouped_models.recommended, vec!["tau/claude"]);
        assert_models_eq(
            grouped_models.all.values().flatten().cloned().collect(),
            vec!["tau/claude", "tau/gemini", "openai/gpt-4", "openai/gpt-3.5"],
        );
    }
}
