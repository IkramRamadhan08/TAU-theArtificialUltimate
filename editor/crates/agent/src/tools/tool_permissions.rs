...
/// `.tau/settings.json` where `.tau` is a symlink to an external directory.
///
/// Note: intermediate directories *can* be symlinks (not just leaf entries),
/// so we must walk the full ancestor chain. For example:
///   `ln -s /external/config /project/.tau`
/// makes `.tau` an intermediate symlink directory.
...
/// either inside a `.tau/` local-settings directory, inside `.agents/skills/`, or inside
...
/// `.tau/` and `.agents/skills/` protections against the canonical form
...
///   2. Intra-project symlinks, e.g. a symlink `safe -> .tau` followed
...
/// like `~/projects/.tau/foo` doesn't classify every file inside it as
/// `.tau/` local-settings — only files that have `.tau` (or
