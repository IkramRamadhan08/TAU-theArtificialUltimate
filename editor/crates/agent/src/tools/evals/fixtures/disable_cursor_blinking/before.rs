...
//! The main point of interest in this crate is [`Editor`] type, which is used in every other Tau part as a user input element.
...
/// Tau's primary implementation of text input, allowing users to edit a [`MultiBuffer`].
///
...
impl Editor {
    ...
    pub fn new_file(
        ...
        Self::new_in_workspace(workspace, window, cx).detach_and_prompt_err(
            ...
            |e, _, _| match e.error_code() {
                ...
                "The remote instance of Tau does not support this yet. It must be upgraded to {}",
    ...
    fn trigger_on_type_formatting(
        ...
        // OnTypeFormatting returns a list of edits, no need to pass them between Tau instances,
        ...
        let result = h_flex()
            ...
            .when(!has_keybind, |el| {
                ...
                el.bg(status_colors.error_background)
                    .border_color(status_colors.error.opacity(0.6))
                    .pl_2()
                    .child(Icon::new(IconName::TauPredictError).color(Color::Error))
                    .cursor_default()
                    .hoverable_tooltip(move |_window, cx| {
                        cx.new(|_| MissingEditPredictionKeybindingTooltip).into()
                    })
                ...
                h_flex()
                    ...
                    .on_click(cx.listener(|this, _event, window, cx| {
                        ...
                        window.dispatch_action(
                            tau_actions::OpenTauPredictOnboarding.boxed_clone(),
                            cx,
                        );
                    }))
                    .child(
                        ...
                        h_flex()
                            ...
                            .child(div().px_1p5().child(match &prediction.completion {
                                InlineCompletion::Move { target, snapshot } => {
                                    ...
                                    {
                                        Icon::new(IconName::TauPredictDown)
                                    } else {
                                        Icon::new(IconName::TauPredictUp)
                                    }
                                }
                                InlineCompletion::Edit { .. } => Icon::new(IconName::TauPredict),
                            }))
                            .child(
                                h_flex()
                                    .gap_1()
        ...
        match &completion.completion {
            ...
            InlineCompletion::Edit {
                ...
                let left = if first_edit_row != cursor_point.row {
                    ...
                    Icon::new(IconName::TauPredict).into_any_element()
                };
                ...
                self.selection_history.transaction(transaction_id).cloned()
            {
                ...
                log::error!(
                    "No entry in selection_history found for undo. \
                     ...
                     https://github.com/tau-industries/tau/issues/22692"
                );
            }
            self.request_autoscroll(Autoscroll::fit(), cx);
    ...
    pub fn copy_path(
        &mut self,
        _: &tau_actions::workspace::CopyPath,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub fn copy_relative_path(
        &mut self,
        _: &tau_actions::workspace::CopyRelativePath,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
                ...
                .child(
                    h_flex()
                        ...
                        .child(Button::new("open-keymap", "Assign Keybinding").size(ButtonSize::Compact).on_click(|_ev, window, cx| {
                            window.dispatch_action(tau_actions::OpenKeymap.boxed_clone(), cx)
                        }))
                        .child(Button::new("see-docs", "See Docs").size(ButtonSize::Compact).on_click(|_ev, _window, cx| {
                            cx.open_url("https://tau.dev/docs/completions#edit-predictions-missing-keybinding");
                        })),
                )
        })
    }
