...
mod tests {
    ...
    async fn test_streaming_authorize(cx: &mut TestAppContext) {
        ...
        let _auth = cx
            .update(|cx| edit_tool.authorize(&PathBuf::from(".tau/settings.json"), &stream_tx, cx));

        let event = stream_rx.expect_authorization().await;
        assert_eq!(
            event.tool_call.fields.title,
            Some("Edit `.tau/settings.json` (local settings)".into())
        );
        ...
        // Test 3: Relative path without .tau should not require confirmation
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        ...
        // Test 4: Path with .tau in the middle should require confirmation
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        let _auth = cx.update(|cx| {
            edit_tool.authorize(&PathBuf::from("root/.tau/tasks.json"), &stream_tx, cx)
        });
        let event = stream_rx.expect_authorization().await;
        assert_eq!(
            event.tool_call.fields.title,
            Some("Edit `root/.tau/tasks.json` (local settings)".into())
        );
        ...
        // 5.1: .tau/settings.json is a sensitive path — still prompts
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        let _auth = cx
            .update(|cx| edit_tool.authorize(&PathBuf::from(".tau/settings.json"), &stream_tx, cx));
        let event = stream_rx.expect_authorization().await;
    ...
    /// `.tau/foo/../../safe.json` similarly sidesteps the consecutive-
    /// component scan for `.tau/`, so the canonical-path recheck has to
    /// catch it. (We escape *out* of `.tau/` here and back in via `..`,
    ...
    async fn test_streaming_authorize_blocks_dotdot_settings_bypass(cx: &mut TestAppContext) {
        ...
        let _auth = cx.update(|cx| {
            edit_tool.authorize(
                &PathBuf::from(path!("/root/.tau/foo/../settings.json")),
                &stream_tx,
                cx,
            )
        });
        ...
        assert!(
            ...
            "`..` traversal into .tau must still prompt: {:?}",
            event.tool_call.fields.title,
        );
    }

    /// An intra-project symlink like `safe -> .tau` keeps a path's
    /// raw components clean of `.tau`, and `resolve_project_path`
    ...
    async fn test_streaming_authorize_blocks_intra_project_symlink_bypass(cx: &mut TestAppContext) {
        ...
        .await;
        fs.insert_symlink(path!("/root/safe"), PathBuf::from(".tau"))
            .await;
        let (edit_tool, _project, _action_log, _fs, _thread) =
            setup_test_with_fs(cx, fs, &[path!("/root").as_ref()]).await;
        ...
        assert!(
            event
                ...
                .as_deref()
                .is_some_and(|title| title.ends_with("(local settings)")),
            "Intra-project symlink to .tau must still prompt: {:?}",
            event.tool_call.fields.title,
        );
    }
    ...
    async fn test_streaming_needs_confirmation_with_multiple_worktrees(cx: &mut TestAppContext) {
        ...
        let test_cases = vec![
            ...
            (
                "shared/.tau/settings.json",
                true,
                ".tau file in third worktree",
            ),
    ...
    async fn test_streaming_needs_confirmation_with_different_modes(cx: &mut TestAppContext) {
        ...
        for _mode in modes {
            // Test .tau path with different modes
            let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
            let _auth = cx.update(|cx| {
                edit_tool.authorize(&PathBuf::from("project/.tau/settings.json"), &stream_tx, cx)
            });
