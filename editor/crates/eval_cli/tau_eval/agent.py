"""Harbor agent wrapper for Tau's eval-cli binary.
...
class ZedAgent(BaseInstalledAgent):
    """Runs Tau's headless AI agent (eval-cli) to solve tasks.

    The eval-cli binary boots a headless GPUI application and uses the same
    NativeAgent + AcpThread pipeline as the production Tau editor, driving
    ...
    async def _install_lsps(self, environment: BaseEnvironment) -> None:
        """Pre-install language servers so Tau doesn't download them at runtime.
        ...
        for name, cmd in lsp_installs:
            try:
                await self.exec_as_agent(
                    environment,
                    command=(
                        'ZED_DATA_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/tau"; '
                        + cmd
                    ),
                )
            except Exception as exc:
        ...
        try:
            await self.exec_as_agent(
                ...
                command=(
                    ...
                    'mkdir -p "$ESLINT_DIR"; '
                    'curl -fsSL "https://github.com/tau-industries/vscode-eslint/archive/refs/tags/release/2.4.4.tar.gz" '
                    '| tar -xz -C "$ESLINT_DIR"; '
