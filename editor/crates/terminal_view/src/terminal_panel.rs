...
use tau_actions::assistant::InlineAssist;
                            ...
                            .menu(move |window, cx| {
                                ...
                                let menu = ContextMenu::build(window, cx, |menu, _, _| {
                                    menu.context(focus_handle.clone())
                                        ...
                                        .action(
                                            "Spawn Task",
                                            tau_actions::Spawn::modal().boxed_clone(),
                                        )
                                });
        ...
        let popover_menu = PopoverMenu::new("settings-popover")
            ...
            .menu(move |window, cx| {
                Some(ContextMenu::build(window, cx, |context_menu, _, _| {
                    context_menu
                        .action("Open Settings", tau_actions::OpenSettings.boxed_clone())
                        .action(
                            "Edit settings.json",
                    ...
                    .child(SplitButton::new(
                        ButtonLike::new("open-settings-ui")
                            ...
                            .on_click(|_, window, cx| {
                                window.dispatch_action(tau_actions::OpenSettings.boxed_clone(), cx);
                            }),
...
impl Render for InlineAssistTabBarButton {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        IconButton::new("terminal_inline_assistant", IconName::TauAssistant)
            .icon_size(IconSize::Small)
...
mod tests {
    ...
    fn test_prepare_script_like_task() {
        let user_command = r#"REPO_URL=$(git remote get-url origin | sed -e \"s/^git@\\(.*\\):\\(.*\\)\\.git$/https:\\/\\/\\1\\/\\2/\"); COMMIT_SHA=$(git log -1 --format=\"%H\" -- \"${TAU_RELATIVE_FILE}\"); echo \"${REPO_URL}/blob/${COMMIT_SHA}/${TAU_RELATIVE_FILE}#L${TAU_ROW}-$(echo $(($(wc -l <<< \"$TAU_SELECTED_TEXT\") + $TAU_ROW - 1)))\" | xclip -selection clipboard"#.to_string();
