...
#[cfg(windows)]
impl MasterProcess {
    const CONNECTION_ESTABLISHED_MAGIC: &str = "TAU_SSH_CONNECTION_ESTABLISHED";
    ...
    pub fn new(
        ...
        master_process
            ...
            .env("SSH_ASKPASS_REQUIRE", "force")
            .env("SSH_ASKPASS", askpass_script_path)
            .env("TAU_ASKPASS_SOCKET", askpass_socket_path)
            .args(additional_args)
            .arg(destination)
            .args(args);
...
impl RemoteConnection for SshRemoteConnection {
    ...
    fn start_proxy(
        ...
        const VARS: [&str; 3] = ["RUST_LOG", "RUST_BACKTRACE", "TAU_GENERATE_MINIDUMPS"];
...
/// given destination. See: https://github.com/tau-industries/tau/issues/45271
#[cfg(not(windows))]
...
impl SshRemoteConnection {
    ...
    async fn ensure_server_binary(
        ...
        let wanted_version = cx.update(|cx| match release_channel {
            ...
            ReleaseChannel::Dev => {
                anyhow::bail!(
                    "TAU_BUILD_REMOTE_SERVER is not set and no remote server exists at ({:?})",
                    dst_path
                )
            }
    ...
    async fn platform_windows(&self, shell: ShellKind) -> Result<RemotePlatform> {
        ...
        Ok(RemotePlatform {
            ...
            arch: match output.trim() {
                ...
                arch => anyhow::bail!(
                    "Prebuilt remote servers are not yet available for windows-{arch}. See https://tau.dev/docs/remote-development"
                ),
            },
        })
    }
...
mod tests {
    ...
    fn test_build_command_quotes_env_assignment() -> Result<()> {
        let mut input_env = HashMap::default();
        input_env.insert("TAU$(echo foo)".to_string(), "value".to_string());
        ...
        assert!(
            remote_command.contains("exec env 'TAU$(echo foo)=value' remote_program"),
