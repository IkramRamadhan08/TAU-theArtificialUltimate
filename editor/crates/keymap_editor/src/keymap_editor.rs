...
use tau_actions::{ChangeKeybinding, OpenKeymap};
                                ...
                                h_flex()
                                    ...
                                    .child(
                                        Button::new("edit-in-json", "Edit in JSON")
                                            ...
                                            .key_binding(
                                                ui::KeyBinding::for_action_in(&tau_actions::OpenKeymapFile, &focus_handle, cx)
                                                    .map(|kb| kb.size(rems_from_px(10.))),
                                            )
                                            .on_click(|_, window, cx| {
                                                window.dispatch_action(
                                                    tau_actions::OpenKeymapFile.boxed_clone(),
                                                    cx,
                                                );
                                            })
                                    )
    ...
    let language_task = workspace
        .read_with(cx, |workspace, cx| {
            workspace
                ...
                .read(cx)
                .languages()
                .language_for_name("Tau Keybind Context")
        })
        .context("Failed to load Tau Keybind Context language")
        .log_err();
...
mod tests {
    ...
    fn binding_is_unbound_by_unbind_respects_precedence() {
        let binding = gpui::KeyBinding::new("tab", tau_actions::OpenKeymap, None);
        let unbind =
