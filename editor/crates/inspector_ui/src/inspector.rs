...
pub fn init(app_state: Arc<AppState>, cx: &mut App) {
    cx.on_action(|_: &tau_actions::dev::ToggleInspector, cx| {
    ...
    v_flex()
        ...
        .child(
            div()
                ...
                .tooltip(Tooltip::text("Click to open by running Tau CLI"))
...
async fn open_zed_source_location(
    ...
    let mut path = Path::new(env!("TAU_REPO_DIR")).to_path_buf();
    ...
    let output = new_command("tau")
        .arg(&path_arg)
        .output()
        .await
        .with_context(|| format!("running tau to open {path_arg} failed"))?;

    if !output.status.success() {
        Err(anyhow!(
            "running tau to open {path_arg} failed with stderr: {}",
