...
const FRONT_MATTER_COMMENT: &str = "<!-- TAU_META {} -->";
...
fn handle_postprocessing() -> Result<()> {
    ...
    let tau_html = output.remove("tau-html").expect("tau-html output defined");
    let default_description = tau_html
        .get("default-description")
        .expect("Default description not found")
        .as_str()
        .expect("Default description not a string")
        .to_string();
    let default_title = tau_html
        .get("default-title")
    ...
    output.insert("html".to_string(), tau_html);
