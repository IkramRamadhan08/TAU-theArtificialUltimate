...
// If the tau binary doesn't use anything in this crate, it will be optimized away
...
/// Opens a tau:// URL within the application.
#[derive(Clone, PartialEq, Deserialize, JsonSchema, Action)]
#[action(namespace = tau)]
#[serde(deny_unknown_fields)]
...
#[action(namespace = tau, no_json, no_register)]
...
actions!(
    ...
    [
        ...
        #[action(deprecated_aliases = ["tau_actions::OpenSettingsEditor"])]
        OpenSettings,
        /// Opens the settings JSON file.
        #[action(deprecated_aliases = ["tau_actions::OpenSettings"])]
        OpenSettingsFile,
        /// Opens project-specific settings.
        #[action(deprecated_aliases = ["tau_actions::OpenProjectSettings"])]
        OpenProjectSettings,
        /// Opens the default keymap file.
        OpenDefaultKeymap,
        /// Opens the user keymap file.
        #[action(deprecated_aliases = ["tau_actions::OpenKeymap"])]
        OpenKeymapFile,
        /// Opens the keymap editor.
        #[action(deprecated_aliases = ["tau_actions::OpenKeymapEditor"])]
        OpenKeymap,
...
pub struct CreateWorktree {
    /// When this is None, Tau will randomly generate a worktree name.
...
pub struct Rerun {
    ...
    /// If it is not, environment variables such as TAU_COLUMN, TAU_FILE are gonna be the same as in the last execution of a task
