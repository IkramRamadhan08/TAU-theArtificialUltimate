//! Baseline interface of Tasks in Tau: all tasks in Tau are intended to use those for implementing their own logic.
...
pub use tau_actions::RevealTarget;
...
/// Contains all information needed by Tau to spawn a new terminal tab for the given task.
...
/// Variables, available for use in [`TaskContext`] when a Tau's [`TaskTemplate`] gets resolved into a [`ResolvedTask`].
...
impl FromStr for VariableName {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let without_prefix = s.strip_prefix(TAU_VARIABLE_NAME_PREFIX).ok_or(())?;
        let value = match without_prefix {
            ...
            _ => {
                if let Some(custom_name) =
                    without_prefix.strip_prefix(TAU_CUSTOM_VARIABLE_NAME_PREFIX)
                {
...
pub const TAU_VARIABLE_NAME_PREFIX: &str = "TAU_";
const TAU_CUSTOM_VARIABLE_NAME_PREFIX: &str = "CUSTOM_";

impl std::fmt::Display for VariableName {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::File => write!(f, "{TAU_VARIABLE_NAME_PREFIX}FILE"),
            Self::Filename => write!(f, "{TAU_VARIABLE_NAME_PREFIX}FILENAME"),
            Self::RelativeFile => write!(f, "{TAU_VARIABLE_NAME_PREFIX}RELATIVE_FILE"),
            Self::RelativeDir => write!(f, "{TAU_VARIABLE_NAME_PREFIX}RELATIVE_DIR"),
            Self::Dirname => write!(f, "{TAU_VARIABLE_NAME_PREFIX}DIRNAME"),
            Self::Stem => write!(f, "{TAU_VARIABLE_NAME_PREFIX}STEM"),
            Self::WorktreeRoot => write!(f, "{TAU_VARIABLE_NAME_PREFIX}WORKTREE_ROOT"),
            Self::Symbol => write!(f, "{TAU_VARIABLE_NAME_PREFIX}SYMBOL"),
            Self::Row => write!(f, "{TAU_VARIABLE_NAME_PREFIX}ROW"),
            Self::Column => write!(f, "{TAU_VARIABLE_NAME_PREFIX}COLUMN"),
            Self::SelectedText => write!(f, "{TAU_VARIABLE_NAME_PREFIX}SELECTED_TEXT"),
            Self::Language => write!(f, "{TAU_VARIABLE_NAME_PREFIX}LANGUAGE"),
            Self::RunnableSymbol => write!(f, "{TAU_VARIABLE_NAME_PREFIX}RUNNABLE_SYMBOL"),
            Self::PickProcessId => write!(f, "{TAU_VARIABLE_NAME_PREFIX}PICK_PID"),
            Self::MainGitWorktree => write!(f, "{TAU_VARIABLE_NAME_PREFIX}MAIN_GIT_WORKTREE"),
            Self::GitSha => write!(f, "{TAU_VARIABLE_NAME_PREFIX}GIT_SHA"),
            Self::GitShaShort => write!(f, "{TAU_VARIABLE_NAME_PREFIX}GIT_SHA_SHORT"),
            Self::GitRepositoryName => write!(f, "{TAU_VARIABLE_NAME_PREFIX}GIT_REPOSITORY_NAME"),
            Self::GitRepositoryPath => write!(f, "{TAU_VARIABLE_NAME_PREFIX}GIT_REPOSITORY_PATH"),
            Self::GitRef => write!(f, "{TAU_VARIABLE_NAME_PREFIX}GIT_REF"),
            Self::Custom(s) => write!(
                f,
                "{TAU_VARIABLE_NAME_PREFIX}{TAU_CUSTOM_VARIABLE_NAME_PREFIX}{s}"
            ),
        }
    }
}

/// Container for predefined environment variables that describe state of Tau at the time the task was spawned.
...
/// Keeps all Tau-related state inside, used to produce a resolved task out of its template.
...
pub struct TaskContext {
    ...
    /// Environment variables obtained when loading the project into Tau.
...
impl EnvVariableReplacer {
    ...
    // Replaces occurrences of VsCode-specific environment variables with Tau equivalents.
    fn replace(&self, input: &str) -> String {
        shellexpand::env_with_context_no_errors(&input, |var: &str| {
            ...
            if let Some(substitution) = self.variables.get(variable_name) {
                // Got a VSCode->Tau hit, perform a substitution
