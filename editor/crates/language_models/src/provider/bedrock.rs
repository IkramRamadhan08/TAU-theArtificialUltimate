...
// These environment variables all use a `TAU_` prefix because we don't want to overwrite the user's AWS credentials.
static TAU_BEDROCK_ACCESS_KEY_ID_VAR: LazyLock<EnvVar> = env_var!("TAU_ACCESS_KEY_ID");
static TAU_BEDROCK_SECRET_ACCESS_KEY_VAR: LazyLock<EnvVar> = env_var!("TAU_SECRET_ACCESS_KEY");
static TAU_BEDROCK_SESSION_TOKEN_VAR: LazyLock<EnvVar> = env_var!("TAU_SESSION_TOKEN");
static TAU_AWS_PROFILE_VAR: LazyLock<EnvVar> = env_var!("TAU_AWS_PROFILE");
static TAU_BEDROCK_REGION_VAR: LazyLock<EnvVar> = env_var!("TAU_AWS_REGION");
static TAU_AWS_ENDPOINT_VAR: LazyLock<EnvVar> = env_var!("TAU_AWS_ENDPOINT");
static TAU_BEDROCK_BEARER_TOKEN_VAR: LazyLock<EnvVar> = env_var!("TAU_BEDROCK_BEARER_TOKEN");
...
impl State {
    ...
    fn load_static_credentials(
        ...
        cx.spawn(async move |this, cx| {
            ...
            let (auth, from_env) = if let Some(bearer_token) = &TAU_BEDROCK_BEARER_TOKEN_VAR.value {
            ...
            } else if let Some(access_key_id) = &TAU_BEDROCK_ACCESS_KEY_ID_VAR.value {
                if let Some(secret_access_key) = &TAU_BEDROCK_SECRET_ACCESS_KEY_VAR.value {
                    if !access_key_id.is_empty() && !secret_access_key.is_empty() {
                        let session_token = TAU_BEDROCK_SESSION_TOKEN_VAR
                            .value
                            .as_deref()
                            .filter(|s| !s.is_empty())
                            .map(|s| s.to_string());
    ...
    fn get_region(&self) -> String {
        ...
        if let Some(region) = TAU_BEDROCK_REGION_VAR.value.as_deref() {
            if !region.is_empty() {
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let configured_label = match &auth {
            ...
            Some(BedrockAuth::IamCredentials { .. }) if env_var_set => {
                format!(
                    ...
                    TAU_BEDROCK_ACCESS_KEY_ID_VAR.name, TAU_BEDROCK_SECRET_ACCESS_KEY_VAR.name
                )
            }
            ...
            Some(BedrockAuth::ApiKey { .. }) if env_var_set => {
                format!(
                    ...
                    TAU_BEDROCK_BEARER_TOKEN_VAR.name
                )
            }
        ...
        let tooltip_label = if env_var_set {
            Some(format!(
                ...
                TAU_BEDROCK_ACCESS_KEY_ID_VAR.name,
                TAU_BEDROCK_SECRET_ACCESS_KEY_VAR.name,
                TAU_BEDROCK_SESSION_TOKEN_VAR.name,
                TAU_BEDROCK_BEARER_TOKEN_VAR.name
            ))
        ...
        v_flex()
            ...
            .child(Label::new("To use TAU's agent with Bedrock, you can set a custom authentication strategy through your settings file or use static credentials."))
...
impl ConfigurationView {
    fn render_static_credentials_ui(&self) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(
                Label::new(format!(
                    "You can also set the {}, {} and {} environment variables (or {} for Bedrock API Key authentication) and restart Tau.",
                    TAU_BEDROCK_ACCESS_KEY_ID_VAR.name,
                    TAU_BEDROCK_SECRET_ACCESS_KEY_VAR.name,
                    TAU_BEDROCK_REGION_VAR.name,
                    TAU_BEDROCK_BEARER_TOKEN_VAR.name
                ))
            ...
            .child(
                Label::new(format!(
                    ...
                    TAU_AWS_PROFILE_VAR.name,
                    TAU_AWS_ENDPOINT_VAR.name,
                    TAU_BEDROCK_SESSION_TOKEN_VAR.name
                ))
                .size(LabelSize::Small)
                .color(Color::Muted)
